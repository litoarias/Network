...
public class MockURLProtocol: URLProtocol {
    ...
    public override func startLoading() {
        defer {
            DispatchQueue.global().asyncAfter(deadline: .now() + .seconds(Self.simulatedDelay)) { [weak self] in
                guard let self = self else { return }
                self.client?.urlProtocolDidFinishLoading(self)
            }
        }
        let foundRequest = MockURLProtocol.mockRequests.first { [unowned self] in
